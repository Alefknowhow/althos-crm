...
import { Plus, Pencil, Trash2, Users, Stethoscope, DoorOpen, User as UserIcon } from 'lucide-react'
...
const EMPTY_PROFESSIONAL: ClinicProfessionalInput = {
  contato_id: '', specialty_id: null, registration_no: null, commission_pct: null,
}
...
function ProfessionalsPanel({
  ...
  const [selectedContato, setSelectedContato] = useState<{ id: string; name: string; email?: string | null; phone?: string | null } | null>(null)
  ...
  function openNew() { setEditingId(null); setSelectedContato(null); setDraft(EMPTY_PROFESSIONAL); setDialogOpen(true) }
  function openEdit(p: ClinicProfessional) {
    setEditingId(p.id)
    setSelectedContato(p.contato_id ? { id: p.contato_id, name: p.name, email: p.email, phone: p.phone } : null)
    setDraft({ contato_id: p.contato_id || '', specialty_id: p.specialty_id, registration_no: p.registration_no, commission_pct: p.commission_pct })
    setDialogOpen(true)
  }
              ...
              <form onSubmit={handleSave} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <LeadCombobox
                    name="contato_id"
                    orgSlug={orgSlug}
                    defaultLead={selectedContato}
                    placeholder="Buscar contato pelo nome, e-mail ou telefone..."
                    onChange={c => { setSelectedContato(c); setDraft(d => ({ ...d, contato_id: c?.id || '' })) }}
                  />
                  <p className="text-[11px] text-muted-foreground">
                    Nome, foto, telefone e e-mail vêm do cadastro do contato — edite lá pra atualizar esses dados.
                  </p>
                </div>
                <div className="space-y-2">
                ...
                <DialogFooter>
                  <Button type="submit" disabled={saving || !draft.contato_id}>{saving ? 'Salvando...' : editingId ? 'Salvar' : 'Criar'}</Button>
                </DialogFooter>
              </form>
            </DialogContent>
          </Dialog>
