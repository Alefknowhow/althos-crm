...
export default async function TasksPage({ params }: { params: { orgSlug: string } }) {
  ...
  const [{ data: tasks }, members] = await Promise.all([
    supabase
      .from('tasks')
      .select('id, title, description, color, status, priority, due_date, duration_minutes, completed_at, created_at, assigned_to, column_id, sale_id, related_entity_type, related_entity_id, leads:contatos(id, name)')
