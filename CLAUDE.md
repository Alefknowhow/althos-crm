...
- **Agenda** (issue #14, set/2026) — módulo único, transversal a qualquer nicho, com três visões na sidebar (`Agenda → Tarefas | Calendário | Projetos`, `components/features/SidebarAgendaAccordion.tsx`), cada uma com permissão própria (`tasks`/`events`/`projects`):
  - **Tarefas** (`/agenda/tarefas`, tabela `tasks`, `actions/tasks-crud.ts`): ação a executar, com responsável/prazo/status. Continua com seu próprio mini-calendário (mês/semana) embutido — não é a mesma coisa da visão Calendário.
  - **Calendário** (`/agenda/calendario`, tabela `events`, `actions/events.ts`): compromissos/eventos que ocupam um intervalo de tempo (`start_at`/`end_at`), conceito distinto de Task e de `appointments` (booking público, ver módulo `agendamentos` abaixo — não foi unificado com Agenda). Campos de sync externo (`meeting_provider`/`external_event_id`/...) são só preparação de schema, sem OAuth/Google/Outlook implementado.
