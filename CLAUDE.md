...
| Ads Tool Layer (Tráfego) | Abstração sobre providers de anúncio, mesmo padrão de `StorageService`/`VoiceProvider`: `lib/ads/index.ts` (`getAdsAdapter(provider)`), `lib/ads/providers/{meta,google}.ts` — Meta embrulha `lib/meta/ads.ts` (leitura), Google é stub (`AdsCapabilityError` em qualquer chamada, sem OAuth/developer token). Capabilities de escrita (`pauseCampaign`/`updateBudget`) só viram `true` com `META_ADS_WRITE_ENABLED=true` (não setada em produção); token do adapter é sempre resolvido server-side (`resolveAdsToken`), nunca exposto a tool/LLM. |
| IA | Anthropic (`@anthropic-ai/sdk`) como motor principal, Google Gemini (`@google/genai`) como alternativa em alguns pontos (qualificação de lead, OCR, transcrição de chamada), ElevenLabs (REST, speech-to-text `scribe_v1`) para transcrever áudio enviado no chat do Copiloto — `lib/ai/speech-to-text.ts`, chave via `getElevenLabsKey()` (`lib/ai/api-key.ts`, env `ELEVENLABS_API_KEY`) |
| MCP | Servidor MCP próprio em `app/api/mcp/route.ts` (`@modelcontextprotocol/sdk`) — expõe tools do CRM (`lib/agent/tools/registry.ts`) para agentes de IA externos (Claude Code, Codex) autenticados via token pessoal gerado em Configurações → Conector MCP (`app/app/[orgSlug]/configuracoes/agentes/`). Cobertura: além das tools bespoke (clientes/campanhas de tráfego, tarefas), um CRUD genérico (`lib/agent/tools/crud.ts` + `modules-*.ts`) expõe list/get/create/update/delete por módulo em todos os nichos (CRM genérico, viagens, clínicas, imóveis, seguros/tráfego) — exceto prontuário clínico (PHI, fora do agente de propósito). `update_*`/`delete_*` sempre exigem `confirm:true`; sem isso devolvem um preview e a tool instrui o agente a confirmar com o usuário no chat antes de repetir a chamada. |
...
| Deploy | Vercel (região `gru1`, ver `vercel.json`) para o Next.js. **IA Sales Coach** (em construção, ver `.harness/tasks/active/ia-sales-coach.md`) tem uma peça de infra fora do Vercel: `services/sales-coach-realtime/` (Node standalone, deploy-alvo Railway) — sustenta o WebSocket persistente com ElevenLabs Scribe v2 Realtime, algo que uma função serverless não aguenta. Mesmo padrão de gap já identificado (e não resolvido) para Voice AI em `lib/voice/ai-conversation.ts`. |
...
- **CRM genérico**: pipeline (`pipelines`/`pipeline_stages`), leads/contatos (`contatos`), formulários públicos, campanhas de marketing.
- **Agenda** (issue #14, set/2026) — módulo único, transversal a qualquer nicho, com três visões na sidebar (`Agenda → Tarefas | Eventos | Projetos`, `components/features/SidebarAgendaAccordion.tsx`), cada uma com permissão própria (`tasks`/`events`/`projects`). Tasks e Events são bases e UIs deliberadamente isoladas: Tarefas não marca a agenda de Eventos, Eventos não aparece em Tarefas.
  - **Tarefas** (`/agenda/tarefas`, tabela `tasks`, `actions/tasks-crud.ts`, `components/features/tasks/`): só lista (grupos Atrasadas/Hoje/Próximas/Concluídas) — sem calendário próprio.
  - **Eventos** (`/agenda/eventos`, tabela `events`, `actions/events.ts`, `components/features/agenda/eventos/`): compromissos com horário marcado (`start_at`/`end_at`), conceito distinto de Task e de `appointments` (booking público, módulo `agendamentos` abaixo — não unificado com Agenda). Visões mês (grade de chips) e semana/dia (`EventsWeekTimeline.tsx` — timeline por hora estilo Google Agenda: arrastar pra criar, cores, arrastar evento existente pra reagendar). Campos de sync externo (`meeting_provider`/`external_event_id`/...) são só preparação de schema, sem OAuth/Google/Outlook implementado.
  - **Projetos** (`/agenda/projetos`, tabelas `projetos`/`projeto_grupos`, `actions/projects.ts`, componentes em `components/features/agenda/projetos/`): camada de organização sobre Tasks (`tasks.project_id`) — generalizada nesta issue pra qualquer nicho, `client_id` agora opcional (projeto pode ser "uso interno", sem contato vinculado). Deixou de ser exclusivo de Agências de Tráfego; ver `#17` (fora deste repo/CLAUDE.md) para o detalhamento do Kanban. Rotas antigas (`/tarefas`, `/agencias-trafego/projetos*`) viram redirect de compatibilidade.
- **Verticais por nicho** (`org.niche`, gated via `lib/niche.ts` + `lib/niche-modules.ts` — 6 chaves em `NicheKey`, cada uma com módulos e permissões próprias):
  ...
  - `trafego` (`isTrafficNiche`): gestão de contas de anúncio de clientes de agência de tráfego (`agencias-trafego/trafego/`). O Kanban de Projetos que vivia aqui agora é o módulo Agenda → Projetos (generalizado); a aba "Projetos" dentro do workspace de um cliente de tráfego continua existindo, só filtrada por esse cliente.
...
### Banco de dados
- `accounts`/`account_members` (modelo conta → N organizações, usado por billing por-conta e pela issue #30 de multi-organização) existiam em produção mas ficaram fora do histórico de migrations até serem reconciliados em `0262_accounts_baseline_reconciliation.sql` — se você notar outra tabela/coluna em uso que não aparece em nenhuma migration rastreada, é sinal do mesmo tipo de drift; audite via MCP do Supabase (`list_tables`/`execute_sql` contra `information_schema`) antes de assumir que a migration mais recente descreve o schema real.
