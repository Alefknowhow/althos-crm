-- Voos: franquias de bagagem (multi-seleção) e classe da cabine.
-- Pai: bloco "Políticas de cancelamento" (texto livre do agente).
alter table public.quotation_flights
  add column if not exists baggage     jsonb not null default '[]'::jsonb,
  add column if not exists cabin_class text;

comment on column public.quotation_flights.baggage is
  'Franquias selecionadas: array de chaves (item_pessoal, mao, despachada)';

alter table public.travel_proposals
  add column if not exists cancellation_html text;

-- RPC atualizada com os campos novos.
