-- Mapa de rota de voos (Cotações → aba Voos) — deriva o mapa direto dos
-- voos já cadastrados (from_code/to_code/leg_type), sem exigir
-- configuração manual extra. Independente do mapa animado de Conteúdo.
alter table travel_proposals
  add column if not exists flights_map_enabled boolean not null default false;
