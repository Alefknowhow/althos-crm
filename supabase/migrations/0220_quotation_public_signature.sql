-- Assinatura destacada no link público da cotação (abaixo da mensagem de
-- encerramento): nome + foto do usuário que preenche, mensagem
-- personalizada e cores de fundo/texto escolhidas por quem edita.
alter table travel_proposals
  ...
  add column if not exists signature_bg_color text,
  add column if not exists signature_text_color text;
