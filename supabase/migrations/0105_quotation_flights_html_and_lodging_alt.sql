-- Aéreo em texto livre (alternativa aos trechos estruturados) — mesmo padrão
-- de itinerary_html: campo rico com imagens coladas/arrastadas.
alter table public.travel_proposals
  add column if not exists flights_html text;

-- Hospedagem alternativa: quando a cotação tem 2+ opções de hotel pro
-- cliente escolher (em vez de circuito com destinos diferentes), cada
-- opção alternativa carrega seu próprio preço.
alter table public.quotation_lodgings
  add column if not exists is_alternative_option boolean not null default false,
  add column if not exists option_price_per_person_cents integer,
  add column if not exists option_total_cents integer;
