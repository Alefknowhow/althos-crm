-- Mapa animado de Cotações (Conteúdo → "Mapa animado") — avião voando da
-- origem até cada parada, com rastro e país pintado com a bandeira.
-- Estrutura própria (não reaproveita origin_label/destinations, que
-- alimentam outros blocos de texto) para não arriscar mudar o formato que
-- já é consumido em outro lugar.
alter table travel_proposals
  add column if not exists animated_map_enabled boolean not null default false,
  add column if not exists animated_map_route jsonb;
...
begin
  ...
  return jsonb_build_object(
    ...
    'animated_map_route', q.animated_map_route,
