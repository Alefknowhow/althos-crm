...
— usado pela fila de resposta manual em Instagram → Comentários.

> O CRM também tem automação de comentário (regra por palavra-chave), mas
> igual à seção 2, **o caso de uso e o vídeo cobrem só o fluxo manual**
> (aba Instagram → Comentários) — a chamada de API é exatamente a mesma
> (publicar resposta pública / disparar DM privada), só muda quem aciona,
> então o manual já demonstra o uso completo da permissão, mais simples de
> gravar.

### Caso de uso (PT)

> Além do Direct, o Althos CRM permite que o dono do negócio responda
> comentários recebidos em suas publicações do Instagram pela aba
> "Comentários" do CRM, onde ficam listados os comentários que ainda não
> foram respondidos, prontos para o atendente responder diretamente pelo
> próprio CRM (com opção de também enviar uma resposta privada complementar
> via DM). A permissão `instagram_business_manage_comments` é usada para:
> (1) receber, via webhook, os comentários feitos nas publicações da conta
> conectada; e (2) publicar a resposta digitada pelo atendente, como
> comentário público e/ou como DM privada. Como a API do Instagram não
> oferece evento de "novo seguidor", esse é o principal canal do CRM para
> capturar e responder o interesse gerado pelas publicações da conta.

### Caso de uso (EN)

> In addition to Direct Messages, Althos CRM lets a business owner reply to
> comments received on their Instagram posts from the CRM's "Comments" tab,
> which lists comments that haven't been answered yet, ready for an agent to
> reply directly from within the CRM (optionally also sending a
> complementary private DM reply). The `instagram_business_manage_comments`
> permission is used to: (1) receive, via webhook, comments made on the
> connected account's posts; and (2) publish the agent's typed reply, as a
> public comment and/or as a private DM. Since the Instagram API does not
> provide a "new follower" event, this is the CRM's main channel to capture
> and respond to interest generated by the account's posts.
...
> estava lá de uma gravação anterior. **E, igual à seção 2, monte a
> gravação com o post real do Instagram e a tela do CRM visíveis juntos**
> (split screen ou alternando janela sem cortar) — não vale comentar por
> fora e só mostrar o resultado no CRM depois.
...
2. De uma segunda conta, **comente numa publicação real** da conta
   conectada — comentário novo, feito na hora da gravação, com o post do
   Instagram visível na tela.
3. No CRM, abra a aba **Instagram → Comentários** e mostre esse comentário
   aparecendo como **pendente**.
4. Digite uma resposta na caixa de texto, marque (ou não) "Também enviar
...
> 4. De uma segunda conta, comente no post.
> 5. No CRM, acesse Instagram → Comentários e confirme que o comentário
>    aparece como pendente.
> 6. Responda pelo CRM e confirme que a resposta aparece no comentário real
>    do Instagram.
