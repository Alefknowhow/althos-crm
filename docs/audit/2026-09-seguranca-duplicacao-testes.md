...
**2.2 — Lógica de rehost/ingestão de mídia duplicada entre WhatsApp e Instagram (revisado)**
Confirmado com leitura completa: `lib/inngest/whatsapp-ingest.ts::downloadAndStoreMedia` e `lib/social/engine-helpers.ts::rehostInboundAttachment` **já delegam corretamente pro `StorageService`** via `uploadSystemFile` — não há violação do invariante de storage do `CLAUDE.md`, nenhum upload direto ao SDK do R2/Supabase Storage por fora. A duplicação real é menor do que o achado original sugeria: WhatsApp precisa de um fetch em 2 passos com header `Authorization` (metadata da Graph API, depois o arquivo — protocolo da Meta exige isso), enquanto Instagram já recebe uma URL direta e público-fetchável no payload — a etapa que diverge é o protocolo de download em si, não a persistência. Forçar as duas num único helper misturaria uma diferença de protocolo genuína só para economizar ~15 linhas idênticas (o próprio `uploadSystemFile`), num caminho de ingestão de mensagem sensível a regressão. **Decisão: não extrair um `lib/media/rehost.ts` — o risco de tocar dois pipelines de mensagem em produção não compensa o ganho de DRY aqui.**
