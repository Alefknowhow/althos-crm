...
- **Recomendação E.2 (Agendamentos)**: decisão de produto confirmada pelo usuário — Agendamentos é Core Extensível de verdade (reunião/atendimento cabe em qualquer nicho, inclusive Viagens, e não é substituído por Reservas). Removida a exclusão `!isTravelNiche` — o link agora aparece pra todo mundo. Catálogo e Vendas continuam ocultos só pra Viagens (mantidos como `GENERIC_ONLY` no registry — Ofertas/Reservas já cobrem esse papel na vertical).
