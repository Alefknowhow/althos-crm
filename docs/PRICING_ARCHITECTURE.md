...
## Fase 6 — lista de benefícios atualizada + Voice vira exclusividade Business (2026-09-13)

Pedido do usuário: atualizar a lista de benefícios ligada aos planos (usada em `/upgrade` e no `CheckoutModal`), confirmar Business R$599/10 usuários, e deixar explícito que disparos de e-mail de campanha consomem Email Credits.

1. **`lib/billing/plan-features.ts`** (`PLAN_FEATURES`) reescrito por completo — antes tinha números da repricing anterior (créditos de IA 300/1200/3000, usuários "1"/"Até 6"/"Ilimitados") e WhatsApp bloqueado no Starter (`false`), que já não refletia a realidade (Starter tem WhatsApp com teto de uso desde a migration `0155`). Nova tabela segue a estrutura de `docs/PRICING_ARCHITECTURE.md` § 2: usuários incluídos 2/5/10, usuário adicional R$39/49/59, Althos Credits 500/2.500/7.500, CRM/Pipeline/Contatos/Tarefas/Agenda/Conversas/Instagram/Formulários/IA básica em todos os planos, Automações/Agentes de IA/Financeiro/Produtos/Integrações/API/MCP a partir do Pro, Voice AI/SMS/múltiplas unidades/permissões avançadas/dashboards avançados/webhooks/auditoria/suporte prioritário só no Business. E-mail marketing exibido como "Consome Email Credits" em todos os planos (não é mais um boolean incluído/bloqueado — é consumo à parte, já implementado desde a sessão anterior em `lib/inngest/functions.ts::sendEmail`, confirmado que TODO envio de e-mail — incluindo disparos de campanha — já passa por `consumeEmailCredits()`).
2. **Voice AI vira exclusividade do Business** (migration `0247`) — antes `plans.features.voice` e `PLAN_FEATURES.pro.voice` (`lib/plans/config.ts`) eram `true` para Pro também, divergindo da lista de benefícios original que o usuário definiu (seção 2 do pedido de repricing: Voice só aparece no Business). Corrigido nos dois lugares (DB `plans.features` + mirror TS) — sem clientes ativos, mudança de entitlement segura. Copy do paywall (`VoicePaywall.tsx`) também corrigida ("planos Pro e Business" → "plano Business").
3. **WhatsApp "1 número" / "múltiplos números" é oferta comercial, não enforcement ainda** — o código hoje só limita volume mensal de mensagens (`organizations.limit_whatsapp_monthly`), não quantidade de conexões WhatsApp por plano. Documentado aqui para não ser esquecido: se o produto decidir cobrar/travar por número de conexão no futuro, precisa de uma coluna/limite novo — não existe hoje.

## Reavaliação do teste grátis: garantia de reembolso de 14 dias (2026-09-13)

Substitui o modelo anterior ("teste grátis de 15 dias, sem cartão, sem reembolso depois") por: **assina e paga na hora (Pix ou cartão), com reembolso garantido de 100% se cancelar em até 14 dias**. Decisão do usuário, aplicada em toda a copy do site (`/planos`, homepage, FAQ, páginas de nicho) e no fluxo de cadastro.

- **Fluxo de cadastro**: `actions/organization-setup.ts::createOrganization` passa a redirecionar para `/app/[orgSlug]/upgrade` (escolher plano + pagar, reaproveitando `CheckoutModal`) em vez de direto pro app. **Isto é só um redirecionamento** — a conta ainda é criada com um trial funcional de 14 dias (era 15) por baixo, caso o usuário feche a aba antes de pagar. Um GATE que bloqueie de fato o acesso ao app até a confirmação do pagamento **não foi implementado** — ver Pendências.
- **Business deixou de ser "sob consulta"**: agora tem preço público fixo (R$599/mês) e passa pelo checkout normal, igual Starter/Pro — em `components/site/PricingPlans.tsx`, `components/landing/AlthosHomePricing.tsx`, `app/app/[orgSlug]/upgrade/page.tsx` e `components/features/billing/CheckoutModal.tsx`.
- **Reembolso automático**: `lib/asaas/client.ts::refundPayment()` (novo — chama `POST /payments/{id}/refund` da Asaas) + `actions/billing.ts::cancelSubscriptionWithRefund()` (novo) — cancela a assinatura E estorna o pagamento mais recente CONFIRMED/RECEIVED, SE a conta estiver dentro da janela de 14 dias. Fora da janela, cai pro cancelamento normal (sem estorno).
- **Rastreamento da janela**: `organizations.trial_ends_at` é reaproveitada (migration `0250`) — para quem JÁ PAGOU (tem `asaas_subscription_id`), deixa de significar "fim do acesso grátis" e passa a significar "até quando é elegível a reembolso automático". Setada em `activatePlanFromWebhook` (14 dias a partir da confirmação do primeiro pagamento). Nova coluna `organizations.refunded_at` registra quando um estorno automático aconteceu.
- **UI**: `app/app/[orgSlug]/assinatura/SubscriptionActions.tsx` mostra "Cancelar e reembolsar" (em vez de "Cancelar assinatura") enquanto a conta estiver dentro da janela de 14 dias, com copy explicando o estorno.

### Pendências desta mudança (não implementadas nesta leva)
- **Gate de acesso real**: hoje o redirecionamento pra `/upgrade` é só uma sugestão de UX — nada impede um usuário recém-cadastrado de navegar direto pra `/app/[slug]/pipeline` e usar o trial de 14 dias sem nunca pagar (mesmo comportamento de antes). Implementar isso exigiria uma mudança de gating mais ampla (ex.: `app/app/[orgSlug]/layout.tsx` bloquear tudo além de `/upgrade`/`/assinatura` pra contas sem `asaas_subscription_id`), que não foi feita por risco de travar prospects que já estão no meio do trial atual.
- **Cadastro via Google OAuth / convite**: o redirecionamento pro `/upgrade` só foi verificado no fluxo de e-mail/senha (`createOrganization`); usuários convidados (`redeemInvite`) corretamente NÃO passam por isso (entram numa org já paga por outra pessoa) — confirmado por leitura de código, não testado end-to-end no navegador.
- **Reembolso de PIX**: a Asaas processa estorno de cartão automaticamente na maioria dos planos, mas estorno de PIX pode depender de configuração específica da conta Asaas — não verificado nesta leva (exigiria uma cobrança real de teste, que não deve ser feita sem necessidade).
