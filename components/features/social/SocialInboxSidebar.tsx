...
export function SocialInboxSidebar({
  ...
  return (
    <div className={`w-full md:w-1/3 md:max-w-[350px] border-r border-border flex-col bg-background ${selectedConversation ? 'hidden md:flex' : 'flex'}`}>
      <div className="px-3 py-2 border-b border-border shrink-0">
        <Input
          value={query}
          onChange={e => setQuery(e.target.value)}
          placeholder="Pesquisar"
          className="h-9 text-sm rounded-xl bg-muted border-none"
        />
      </div>
      <div className="flex-1 overflow-y-auto">
        {filteredConversations.map(c => (
          <div
            ...
            className={`p-3 cursor-pointer hover:bg-muted/50 transition-colors flex gap-3 justify-between items-center ${selectedConversation?.id === c.id ? 'bg-accent' : ''}`}
          >
            ...
            <div className="flex flex-col items-end gap-1.5 shrink-0">
              <span className="flex items-center gap-1">
                ...
                <span className={`text-[10px] font-medium ${c.unread_count > 0 ? 'text-primary' : 'text-muted-foreground'}`}>{formatInboxTime(c.last_message_at)}</span>
              </span>
              {c.unread_count > 0 && (
