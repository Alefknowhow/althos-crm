 ...
 *  dimensão de Kanban. dnd-kit, mesmo padrão de ProjectsBoard.tsx. */
...
function DraggableTaskCard({ task, groupName }: { task: Task; groupName: string | null }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...listeners} {...attributes} className="touch-none rounded-lg border bg-card p-2.5 space-y-1.5 cursor-grab active:cursor-grabbing">
...
export default function ProjectTasksKanban({ orgSlug, tasks, groups }: { orgSlug: string; tasks: Task[]; groups: Group[] }) {
          ...
          return (
            <DroppableColumn key={col.status} status={col.status}>
              ...
              <div className="space-y-2">
                {items.map(t => (
                  <DraggableTaskCard key={t.id} task={t} groupName={t.project_group_id ? groupName.get(t.project_group_id) || null : null} />
                ))}
