...
export default function DashboardTabsShell({
  ...
  clientes: ReactNode
  equipe: ReactNode
  /** Só passado quando a org é do nicho Clínicas — 6ª aba opcional. */
  clinica?: ReactNode
}) {
  const tabCount = clinica ? 6 : 5
  return (
    <Tabs defaultValue="visao-geral" className="space-y-4">
      <div className="sticky top-0 z-20 -mx-3 sm:-mx-5 px-3 sm:px-5 pt-2 -mt-2 pb-2 space-y-2 bg-secondary/40 backdrop-blur supports-[backdrop-filter]:bg-secondary/70">
        ...
        <TabsList className={`grid gap-1 h-auto w-full sm:inline-flex sm:w-auto sm:gap-0 ${tabCount === 6 ? 'grid-cols-6' : 'grid-cols-5'}`}>
