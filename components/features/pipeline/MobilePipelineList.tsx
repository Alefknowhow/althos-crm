...
function MobileLeadCard({
  ...
  return (
    <div
      onClick={onClick}
      className="rounded-[8px] border bg-background p-2.5 active:bg-muted/40 transition-colors"
      style={{ boxShadow: `inset 3px 0 0 0 ${color}` }}
    >
      {/* Title + responsável */}
      <div className="flex items-start justify-between gap-2">
        <p className="text-[13px] font-semibold leading-snug text-foreground line-clamp-2">{lead.name}</p>
        {owner && (
          <span
            title={owner.name || owner.email}
            className={cn('flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-white text-[10px] font-semibold', agentColor(owner.id || owner.email))}
          >
          ...
          <div key={stage.id} className="overflow-hidden rounded-[8px] border bg-card">
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-0.5 text-[11px] tabular-nums text-muted-foreground">{formatCurrency(total)}</div>
              </div>
