          ...
          {(['monthly', 'semestral', 'annual'] as BillingCycle[]).map(c => (
            <button
              ...
              className={`relative rounded-full px-5 py-2 text-[13px] font-semibold transition-colors ${
                cycle === c ? 'text-white' : 'text-slate-500 hover:text-slate-800'
              }`}
            >
              {cycle === c && (
                <motion.span
        ...
        >
          ...
          <div className="mt-5">
            <div className="flex items-end gap-1">
              ...
              <span className="mb-1 text-[13px] text-slate-500">/mês</span>
            </div>
            <p className="mt-1.5 text-[12px] text-slate-500">Gratuito para sempre · sem cartão</p>
          </div>
          ...
          <ul className="mt-5 space-y-2 border-t border-slate-200 pt-5 sm:mt-6 sm:space-y-2.5 sm:pt-6">
            {[
              ...
              <li key={f.label} className="flex items-start gap-2.5">
                ...
                <span className={`text-[13px] ${f.on ? 'text-slate-700' : 'text-slate-400 line-through'}`}>
                  {f.label}
                </span>
              </li>
            ))}
        ...
        {PUBLIC_PLANS.map((plan, i) => {
          ...
          return (
            ...
            >
              ...
              <div className="mt-5">
                <div className="flex items-end gap-1">
                  <span className="text-4xl font-bold tracking-tight text-slate-900">{pricing.perMonthLabel}</span>
                  <span className="mb-1 text-[13px] text-slate-500">/mês</span>
                </div>
                {cycle === 'annual' ? (
                  ...
                  <p className="mt-1.5 text-[12px] text-slate-500">cobrado mensalmente</p>
                )}
              </div>

              <p className="mt-4 text-[13px] leading-relaxed text-slate-600">{plan.description}</p>
