  ...
  try {
    switch (stepDef.type) {
      ...
      case 'send_instagram_dm': {
        const { runSendInstagramDmStep } = await import('./automation-step-instagram')
        const res = await runSendInstagramDmStep(stepDef.config, { supabase, orgId, orgConfig, lead })
        status = res.status
        message = res.message
        sent = res.sent
        break
