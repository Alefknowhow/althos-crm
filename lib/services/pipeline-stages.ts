...
export async function createStageCore(ctx: ActionContext, pipelineId: string, name: string, color?: string) {
  const orgSlug = ctx.org.slug

  const org = ctx.org
  const perm = await ctx.checkPermission(['pipeline'])
...
export async function updateStageCore(ctx: ActionContext, stageId: string, patch: { name?: string; is_won?: boolean; is_lost?: boolean; color?: string }) {
  const orgSlug = ctx.org.slug
...
export async function deleteStageCore(ctx: ActionContext, stageId: string) {
  const orgSlug = ctx.org.slug
