/**
 * Helpers puros de busca de voo (AeroDataBox via RapidAPI) — parsing,
 * formatação e a chamada HTTP em si. Extraído de actions/flight-lookup.ts
 * (que passou do limite de 350 linhas do lint); um arquivo 'use server' só
 * pode exportar async functions, então esses helpers (não-async ou
 * privados) precisam morar num módulo comum, não-'use server'.
 */

export const RAPIDAPI_HOST = 'aerodatabox.p.rapidapi.com'
 ...
 */
export async function fetchOneFlight(
  designator: string,
  date: string,
  key: string,
...
export type FlightStatusResult = {
  designator: string
  /** Status normalizado da AeroDataBox — ver mapeamento em normalizeStatus. */
...
export function normalizeStatus(raw: string | undefined): FlightStatusResult['status'] {
